'''
---DESCRIPTION--
Created at the request of a Muslim VoxCommando user. 
This is a variant of an existing script developed by PrayTimes.org
I added:
* an additional prayer time calculation (last third of the night).
* code at the end, which is used to generate events in VoxCommando to create prayer time alerts.

In your VC command you'll need to pass your latitude, longitude, and time zone info.
For explanation and context, see forum thread:
http://voxcommando.com/forum/index.php?topic=1022.msg21499#msg21499

---CREDITS---
Base code for this Muslim prayer time code courtesy Praytimes.org
...
class PrayTimes():
	...
	def computePrayerTimes(self, times):
		...
		lastthird = 2*((fajr+(24-isha))/3)+isha #added to script last third of the night

		return {
			'imsak': imsak, 'fajr': fajr, 'sunrise': sunrise, 'dhuhr': dhuhr,
			'asr': asr, 'sunset': sunset, 'maghrib': maghrib, 'isha': isha, 'lastthird':lastthird,
		}
...
# Below is the function that is called from VoxCommando. This is the only thing I added to the script above.
# By default, the script above is calculating prayer times using the Muslim World League (MWL) method. It is possible to change this, though.

from System.Collections.Generic import *
from datetime import date

def myPrayTimes(lat,long,utc):
    today = date.today()
    payloads = ''
    times = prayTimes.getTimes(today, (lat, long), utc);
    #vc.callAction("OSD.ShowText","Prayer times for "+str(today)+"&&7000&&-10", None)

    for i in ['Fajr', 'Sunrise', 'Dhuhr', 'Asr', 'Maghrib', 'Isha','LastThird']:
        if i != 'LastThird':
        #vc.callAction("OSD.AddText",i+": "+times[i.lower()], None)
            vc.callAction("VC.StopEventTimer","praytime."+i,None)
            vc.callAction("VC.SetEventTimer",times[i.lower()]+"&&praytime."+i+"&&"+i+"&&"+times[i.lower()], None)        
        else:
            vc.callAction("VC.StopEventTimer","praytime."+'Last Third',None)
            vc.callAction("VC.SetEventTimer",times[i.lower()]+"&&praytime."+'Last Third'+"&&"+"Last third of the night"+"&&"+times[i.lower()], None) 
